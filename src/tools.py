...
Nơi khai báo tất cả các "món đồ nghề" mà Cupid ReAct Agent có thể gọi.
"""

import json
import random

# Giả lập cơ sở dữ liệu hồ sơ người dùng
USER_DATABASE = {
    "anh": {
        "name": "Anh",
        "gender": "Nam",
...
# Phân nhóm nguyên tố của 12 cung hoàng đạo để tính toán độ tương thích
ZODIAC_ELEMENTS = {
...
# Danh mục địa điểm hẹn hò đề xuất theo sở thích
...
def get_user_profile(name: str) -> str:
    """
    Tra cứu thông tin hồ sơ chi tiết của một người dùng bao gồm cung hoàng đạo, sở thích, giới tính và năm sinh.
    
    Args:
        name (str): Tên của người dùng cần tra cứu (ví dụ: 'Anh', 'Mai', 'Linh', 'Vy')
        
    Returns:
        str: Chuỗi JSON chứa thông tin hồ sơ chi tiết hoặc chuỗi thông báo lỗi (bắt đầu bằng LỖI:).
    """
    if not name:
...
def check_zodiac_compatibility(sign1: str, sign2: str) -> str:
    """
    Kiểm tra độ tương thích và phân tích mối quan hệ giữa hai cung hoàng đạo.
    ...
    Returns:
        str: Chuỗi JSON chứa kết quả đánh giá điểm số (%) và nhận xét mối quan hệ.
    """
    ...
    if s1 not in ZODIAC_ELEMENTS or s2 not in ZODIAC_ELEMENTS:
        ...
        return f"LỖI: Cung hoàng đạo không hợp lệ hoặc không tồn tại: {', '.join(invalid)}."
        
    # Cấu hình điểm số cho các cặp đôi đặc biệt
    special_pairs = {
    ...
    if pair in special_pairs:
        ...
        if e1 == e2:
            ...
            score = 80
            comment = "Gió thổi bùng lửa. Kích thích tư duy, truyền cảm hứng và giúp nhau cùng tiến bộ."
